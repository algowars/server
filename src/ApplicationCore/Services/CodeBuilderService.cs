...
{
    public async Task<Result<IEnumerable<CodeBuildResult>>> BuildAsync(
        IEnumerable<CodeBuilderContext> contexts
    )
    {
        if (contexts == null || !contexts.Any())
        {
            return Result<IEnumerable<CodeBuildResult>>.Invalid(
                new ValidationError("Context list cannot be null or empty.")
            );
        }
    ...
    private static Result<IEnumerable<CodeBuildResult>>? ValidateContext(CodeBuilderContext context)
    {
        if (context == null)
        {
            return Result<IEnumerable<CodeBuildResult>>.Invalid(
