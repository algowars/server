    ...
    : IQueryHandler<GetAvailableLanguagesQuery, IEnumerable<ProgrammingLanguageDto>>
{
    ...
    )
    {
        try
        {
            var languages = await problemRepository.GetAvailableLanguagesAsync(cancellationToken);

            return Result.Success(
                languages.Select(language => new ProgrammingLanguageDto
                {
                    Id = language.Id,
                    Name = language.Name,
                    Versions = language.Versions.Select(version => new LanguageVersionDto
                    {
                        Id = version.Id,
