...
namespace Algowars.Infrastructure.Persistence.Seeders.Once.Problems;

internal sealed class SeedTwoSumProblem(AlgowarsDbContext context)
    : OnceSeeder(context)
{
    public override string Name => "20260712_SeedTwoSumProblem";

    protected override async Task ExecuteAsync(CancellationToken cancellationToken)
    {
        bool exists = await context.Problems
            .IgnoreQueryFilters()
            .AnyAsync(p => p.Slug == new Slug("two-sum"), cancellationToken);

        if (exists)
        {
            Console.WriteLine("[seed] two-sum already exists, skipping insert.");
            return;
        }
        ...
        ProblemSetup jsSetup = twoSum.AddSetup(jsVersion.Id, "function twoSum(nums, target) {\n    \n}", "twoSum");
        ProblemSetup tsSetup = twoSum.AddSetup(tsVersion.Id, "function twoSum(nums: number[], target: number): number[] {\n    \n}", "twoSum");
        ProblemSetup pySetup = twoSum.AddSetup(pyVersion.Id, "def two_sum(nums: list[int], target: int) -> list[int]:\n    pass", "two_sum");

        context.Problems.Add(twoSum);
        await context.SaveChangesAsync(cancellationToken);

        TestSuite sampleSuite = BuildSampleSuite();
        TestSuite hiddenSuite = BuildHiddenSuite();

        context.TestSuites.AddRange(sampleSuite, hiddenSuite);
        await context.SaveChangesAsync(cancellationToken);

        foreach (Guid setupId in new[] { jsSetup.Id, tsSetup.Id, pySetup.Id })
        {
            ProblemSetup setup = await context.Set<ProblemSetup>()
                .Include(s => s.TestSuites)
                .FirstAsync(s => s.Id == setupId, cancellationToken);
